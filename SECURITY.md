...
| Ruby on Rails exercises | Ruby 4.0.6 (`.ruby-version`)       | Bundler-managed gems per sample app; keep Rails/Botpress gems current.                                         |
...
| Ruby CLI helpers        | Ruby 4.0.6                         | Depend only on the Ruby standard library.                                                                      |
| Python automation       | CPython 3.14.7 (`.python-version`) | Scripts currently rely on the Python standard library; add new packages via per-folder requirements if needed. |
